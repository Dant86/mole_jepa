"""Unit tests for the Epps-Pulley regularizers."""
...
class TestEppsPulleyGaussian:
    def test_output_is_scalar(self, normal_samples: torch.Tensor) -> None:
        reg = regularizers.EppsPulleyGaussian()
...
class TestEppsPulleyLaplace:
    def test_output_is_scalar(self, laplace_samples: torch.Tensor) -> None:
        reg = regularizers.EppsPulleyLaplace()
...
class TestEppsPulleyFactory:
    def test_gaussian_returns_correct_type(self) -> None:
        reg = regularizers.epps_pulley("gaussian")
        assert isinstance(reg, regularizers.EppsPulleyGaussian)

    def test_laplace_returns_correct_type(self) -> None:
        reg = regularizers.epps_pulley("laplace")
        assert isinstance(reg, regularizers.EppsPulleyLaplace)

    def test_unknown_distribution_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown distribution"):
            regularizers.epps_pulley("cauchy")  # type: ignore[arg-type]

    def test_forwards_kwargs(self) -> None:
        reg = regularizers.epps_pulley("gaussian", t_max=5.0, n_integration_points=33)
